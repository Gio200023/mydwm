...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - 2 * sp, 0, tw, bh, 0, stext, 0);
               // tw = m->ww -drawstatusbar(m,bh,stext); /* 2px right padding */                      
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, bh, bh, wdelta + lrpad / 2, (selmon->alttag ? tagsalt[i] : tags[i]), urg & 1 << i);
		if (occ & 1 << i)
                        //con questa cambia il rettangolino che appare in alto se c'è un qualcosa di aperto
...
tile(Monitor *m)
{
	...
	for(i = 0, my = ty = gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + gappx, m->wy + my, mw - 2*bw - gappx*(5-ns)/2, h- 2*bw, bw, 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c) + gappx;
		} else {
			h = (m->wh - ty) / (n - i) - gappx;
			resize(c, m->wx + mw + gappx/ns, m->wy + ty, m->ww - mw - 2*bw - gappx*(5-ns)/2, h - 2*bw, bw, 0 );
			if (ty + HEIGHT(c) < m->wh)
				ty += HEIGHT(c) + gappx;
		}
}
